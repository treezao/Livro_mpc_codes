...
G(1,1) = 1.43/(7.5*s+1)*exp(-5*s);
G(1,2) = -4.5/(13.4*s+1)*exp(-5*s);
G(2,1) = 0*s;
G(2,2) = 5.72/(14.7*s+1);
...
%%% planta nominal
Gnom(1,1) = 1.43/(7.5*s+1)*exp(-s);
Gnom(1,2) = -4.5/(13.4*s+1)*exp(-s);
Gnom(2,1) = 0*s;
Gnom(2,2) = 5.72/(14.7*s+1);
...
nin = 30; %número da iteração inicial
nit = nit+nin;
...
eta = zeros(n,nit); % vetor dos erros de predição
etaf = zeros(n,nit); % vetor dos erros de predição filtrados
...
for k = nin:nit
    ...
    for i = 1:n
        ...
        eta(i,k) = saidas(i,k)-fma{i}(1);
        
        naf = size(Fe{i}.den,2);
        etaf(i,k) = -Fe{i}.den{1}(2:end)*etaf(i,k-1:-1:k-naf)' + ...
                     Fe{i}.num{1}*eta(i,k:-1:k-naf)';
        
        fma{i} = [fma{i}(2:Nss(i)); fma{i}(Nss(i))];
        f = [f; fma{i}(N1(i):N2(i)) + ones(N(i),1)*etaf(i,k)];
    end
...
% print('tanque1_dmcf_erro_s_filtro','-depsc')
