...
%% planta com erro de modelagem
Gr(1,1) = 2.5/(9*s+1)*exp(-s);
Gr(1,2) = -5.5/(11*s+1);
Gr(2,1) = 0;
Gr(2,2) = 4/(17*s+1);

% Definir os modelos que relacionam {saida x,perturbação z};
Gqr(1,1) = 2.1/(13*s+1)*exp(-s);
Gqr(2,1) = -3.12/(14.7*s+1);


%%% discretização do processo
Gzr = c2d(Gr,Ts,'zoh');
Gzqr = c2d(Gqr,Ts,'zoh'); % perts de entrada

atrasoEntradar = totaldelay(Gzr);
	
if(mq>0)
    atrasoPertr = totaldelay(Gzqr);
end
...
for i=1:n
	...
	for j=1:m
        ...
        [numR{i}{j}, denR{i}{j}] = tfdata(Gzr(i,j),'v');
	end
    
	% obtencao das respostas ao degrau das perts
    for j=1:mq
        [numDR{i}{j},denDR{i}{j}]=tfdata(Gzqr(i,j),'v');
	end
end
...
for k = nin:nit
    ...
    for i=1:n
        ...
        for j=1:mq
            ...
            str1 = numDR{i}{j}*perts(j,k-atrasoPertr(i,j):-1:k+1-sizeN-atrasoPertr(i,j))';
...
ta2.Position = [0.2511 0.7651 0.0471 -0.0516];
ta3.Position = [0.5911 0.3355 -0.0665 0.0271];
ta4.Position = [0.6968 0.2813 -0.0957 -0.0335];


% print('tanque_gpc_polic_erro_q','-depsc')
