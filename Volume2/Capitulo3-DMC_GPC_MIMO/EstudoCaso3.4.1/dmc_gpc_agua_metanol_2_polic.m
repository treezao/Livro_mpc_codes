...
for k = nin:nit
    ...
    for i = 1:n
        ...
        for j = 1:mq
            fma{i} = fma{i} + modDegrauQ{i,j}*dq(:,k);
        end
...
for k = nin:nit
    ...
    for i=1:n
        ...
        for j=1:mq
            dqC{i,j}(1,k) = -C{i}(2:end)*dqC{i,j}(k-1:-1:k-nC(i)+1)' + dq(j,k);
            
            ftemp = ftemp + Hq{i,j}*dqC{i,j}(1,k:-1:k-nHq(i,j)+1)';
        end
...
% print('dmc_gpc_agua_metanol_2_r','-depsc')
...
hf.Position = tamfigura;
hl.Position = [0.7365 0.4780 0.2179 0.2371];
ta1.Position = [0.5982 0.8871 -0.0607 -0.0451];
ta2.Position = [0.6214 0.7709 -0.0633 -0.0271];
ta3.Position = [0.4882 0.3967 -0.0671 -0.0465];
ta4.Position = [0.5501 0.2290 -0.0686 0.0335];

% print('dmc_gpc_agua_metanol_2_q','-depsc')
