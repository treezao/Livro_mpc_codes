%%% Simulação comparativa com diferentes limites de iterações do NEPSAC
clear all
close all
clc
addpath('../../../Bibliotecas/')
...
for k=nin:nit
    ...
    contt = 0;
    u0 = entrada(:,k-1)/1000;
    while (dunorm > .1 && cont <= 5) % iterar até obter a resposta dentro do erro especificado
...
for k=nin:nit
    ...
    contt = 0;
    u0 = entrada(:,k-1)/1000;
    while (dunorm > .1 && cont <= 20) % iterar até obter a resposta dentro do erro especificado
        %%% Obtenção da resposta base
        yb = zeros(n,N2max); % resposta base
        yb(:,1) =  modeloCSTRnominal(saida(:,k),entrada(:,k-1),[CAf0;Tf0;Tcf0]) + eta(:,k+1); % yb(k+1|k)
        for i=2:N2max
...
ylim([0.078 0.103])
hl = legend('max it = 5','max it = 10','max it = 20','Referência','Location','SouthEast')
...
print('casoCSTR_comparativo_nepsac2_ref','-depsc')
...
print('casoCSTR_comparativo_nepsac2_pert','-depsc')
...
xlim([0 nit])
% hl = legend('max it = 5','max it = '0','max it = 100','Location','NorthEast')
% 
...
xlim([0 nit])
hl = legend('max it = 5','max it = 10','max it = 20','Location','NorthEast')
% 
...
print('casoCSTR_comparativo_nepsac2_tempos','-depsc')
