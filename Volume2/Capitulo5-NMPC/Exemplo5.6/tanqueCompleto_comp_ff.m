...
for k=2:nit
    ...
    %%% adição da parcela do incremento da perturbação passada na
    %%% resposta livre
    if(~isempty(Hq))
        deltaQ = q(1,k:-1:k-nbq-dq+1)'-q(1,k-1:-1:k-nbq-dq)';
        f = f+ Hq*deltaQ;
    end
...
for k=2:nit
    ...
    R = ref(k)*ones(N2-N1+1,1);
    %%% cálculo da resposta livre;
    saidaModeloGPCnlin(k) = modeloTanque(saidaGPCnlin(k-1),entradaGPCnlin(k-1),q(k-1));
    ...
    f = zeros(N2,1);
    f(1) = modeloTanque(saidaGPCnlin(k),entradaGPCnlin(k-1),q(k)) + etaGPCnlin(k);
    for i=2:N2
        f(i) = modeloTanque(f(i-1),entradaGPCnlin(k-1),q(k)) + etaGPCnlin(k+i);
    end
...
for k=2:nit
    ...
    while (dunorm > 1e-3 && cont <= 20) % iterar até obter a resposta dentro do erro especificado
        ...
        yb(1) = modeloTanque(saida(k),ub(1),q(k)) + eta(k+1); % yb(k+1|k)
        for i=2:N2
            yb(i) = modeloTanque(yb(i-1),ub(i),q(k)) + eta(k+i);
        end
        ...
        for j=1:Nu
            ...
            yot(1) = modeloTanque(saida(k),udelta(1),q(k)) + eta(k+1);
            for i=2:N2
                yot(i) = modeloTanque(yot(i-1),udelta(i),q(k)) + eta(k+i);
            end
...
% print('tanqueCompleto_comp_ff','-depsc')
