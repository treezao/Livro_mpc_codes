...
for k=nin:nit
    ...
    saidaModelo(k) = modeloTanque(saidaModelo(k-1),entrada(k-1),q(k-1));
    ...
    while (dunorm > eps_nep && cont <= nit_nep) % iterar até obter a resposta dentro do erro especificado
        ...
        yb(1) = modeloTanque(saidaModelo(k),ub(1),q(k)); % yb(k+1|k)
        for i=2:N2
            yb(i) = modeloTanque(yb(i-1),ub(i),q(k));
        end
        yb = yb + eta(k+1:k+N2)';
        ...
        for j=1:Nu
            ...
            yot(1) = modeloTanque(saidaModelo(k),udelta(1),q(k));
            for i=2:N2
                yot(i) = modeloTanque(yot(i-1),udelta(i),q(k));
            end
...
% print('tanqueCompleto_comp_nepsac_ff','-depsc')
