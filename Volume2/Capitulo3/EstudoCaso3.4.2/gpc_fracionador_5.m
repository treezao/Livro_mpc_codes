...
h=subplot(3,1,3)
plot(t,du(1,ind),'LineWidth',tamlinha,'Color',cores(1,:))
hold on
plot(t,du(2,ind),'--','LineWidth',tamlinha,'Color',cores(2,:))
plot(t,du(3,ind),':','LineWidth',tamlinha,'Color',cores(3,:))

hl3 = legend('\Delta u_1','\Delta u_2','\Delta u_3','Location','SouthEast')
% ylim([-1.3 1.4])
...
hl.Position = [0.7988 0.6956 0.1643 0.2321];
hl2.Position = [0.8417 0.4413 0.1161 0.1762];
hl3.Position = [0.8292 0.1413 0.1411 0.1762];

% ax = get(gcf,'children');
% ind = find(isgraphics(ax,'Legend'));
% set(gcf,'children',ax([ind:end,1:ind-1]))

% print('gpc_fracionador_5','-depsc')
