...
delta = 1; % ponderação dos erros
lambda = 5; % ponderação do esforço de controle
lambdaU = 0.4;% ponderação do target da ação de controle
...
Kdmcy = inv(G'*Qy*G + Qu + TU'*QU*TU)*G'*Qy;
Kdmcy = Kdmcy(1,:); % vetor de ganhos do erro

Kdmcu = inv(G'*Qy*G + Qu + TU'*QU*TU)*TU'*QU;
Kdmcu = Kdmcu(1,:); % vetor de ganhos do erro do u target
...
h0 = 0; % estado inicial da saída
u0 = 0; % estado inicial da entrada
...
perts(nin+round(8/Ts):end) = 2;

refs = 0*ones(nit,1); % vetor de referências
refs(nin+round(2/Ts):nit) = 3;

refsu = zeros(nit,1); % vetor de referências para a manipulada
...
for k = nin:nit
    ...
    fu = (refsu(k)-entradas(k-1))*ones(Nu,1);
    %%% referências
    R = ones(N2-N1+1,1)*refs(k);

    du(k) = Kdmcy*(R-f) + Kdmcu*fu;
...
hold on
plot(t,52+refs(vx),'-.','LineWidth',tamlinha,'Color',cores(2,:))
...
% xlim([0 26]);
% ylim([48 61])
hl = legend('Caso 2','Referência','Limites')
...
hold on
plot(t,30+refsu(vx),'-.','LineWidth',tamlinha,'Color',cores(2,:))
