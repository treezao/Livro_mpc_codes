...
z = tf('z',Ts);
Gz = 0.1/(z-0.92); % modelo discretizado
...
Nf = 25; % horizonte de modelo do filtro
betaf = 0.7; % constante do filtro do GDMC
...
F = (X(1)*z^2+X(2)*z)/(z-betaf)^2;
modDegrauUF = filter(F.num{1},F.den{1},Gcoef);
...
nin = Nss+1;
nit = 80 + nin; % número de iterações da simulação
...
yfilt = zeros(nit,1); % vetor com as saidas filtras
...
for k = nin:nit
    ...
    yfilt(k) = -F.den{1}(2:end)*yfilt(k-1:-1:k-nf) + F.num{1}*saidas(k:-1:k-nf);
