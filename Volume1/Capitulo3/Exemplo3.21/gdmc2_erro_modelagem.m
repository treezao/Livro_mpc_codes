...
Gzr = 0.1/(z-1.1)/z^4; % modelo discretizado "real"
num = Gzr.num{1}; % numerador do modelo discreto
den = Gzr.den{1}; % denominador do modelo discreto
na = size(den,2)-1; % ordem do denominador
nb = size(num,2)-2; % ordem do numerador
dd = Gzr.inputdelay; % atraso discreto
...
betaf = 0.92; % polo do filtro do gdmc
...
for i = 1:size(Kdmc1,2)
    numc = numc + Kdmc1(i)*F(i).num{1};
end

Fke1 = tf(numc,F(1).den{1},Ts)


denc = conv([1 -1],[1 Kdmc1*H]);
numc = [1 zeros(1,size(denc,2)-1)];


Cr = Fke1*tf(numc,denc,Ts)
...
nin = max(Nss,Nf)+1;
nit = 200 + nin; % número de iterações da simulação
...
% perts(nin+round(50/Ts):end) = 0.2;

refs = 0*ones(nit,1); % vetor de referências
refs(nin+round(4/Ts):end) = .5;
...
ylim([0 1])
% h.YTick = [0 0.5 1 1.5];
hl = legend('GDMC2','Referência','Location','NorthEast')
