...
A = [1 -0.9]; % denominador do modelo do processo
Ar = [1 -0.85]; % denominador "real" do processo
B = [0.3]; % numerador do modelo do processo
Bq = [0 B]; % numerador do modelo da perturbação do processo

d=10; % atraso em relação à manipulada
dq = d; % atraso em relação à perturbação

nb = size(B,2)-1; % ordem do polinômio B
na = size(Ar,2)-1; % ordem do polinômio A
...
N2 = d+20; % horizonte de predição final
...
for k = nin:nit
    ...
    saidas(k) = -Ar(2:end)*saidas(k-1:-1:k-na) ...
