...
Hqp = blkdiag(Hqp, diag(2*lambdaM*ones(N,1)));
...
xi = zeros(nit,N); % vetor com as relaxações
...
for k = nin:nit
    ...
    if(refs(k)==saidas(k))
        X = quadprog(Hqp,[fqp1*(R-f);zeros(N,1)]);
    elseif(refs(k)>=saidas(k))
        Rbar = [-G -eye(N)];
        X = quadprog(Hqp,[fqp1*(R-f);zeros(N,1)], Rbar,-saidas(k)+f);
    else
        Rbar = [G -eye(N)];
        X = quadprog(Hqp,[fqp1*(R-f);zeros(N,1)],Rbar,saidas(k)-f);
    end
