...
ksi = 0.5;
wn = 3/ksi/4.8*1.5;
G = 2*(-0.75*s+1)*wn^2/(s^2+2*ksi*wn*s+wn^2); % modelo em função de transferência em tempo contínuo
...
lambdaM = 1000; % ponderação da relaxação

Nss=150; % horizonte de modelo
...
for k = nin:nit
    ...
    if(refs(k)==saidas(k))
        X = quadprog(Hqp,[fqp1*(R-f);0]);
    elseif(refs(k)>=saidas(k))
        Rbar = [-G -ones(N,1)];
        X = quadprog(Hqp,[fqp1*(R-f);0], Rbar,-saidas(k)+f);
    else
        Rbar = [G -ones(N,1)];
        X = quadprog(Hqp,[fqp1*(R-f);0],Rbar,saidas(k)-f);
    end
    du(k) = X(1);
    xi(k) = X(end); % valor das relaxações
