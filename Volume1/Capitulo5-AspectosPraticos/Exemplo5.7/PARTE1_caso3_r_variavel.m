...
perts(nin+round(2/Ts):end) = 3;
perts(nin+round(20/Ts):end) = 1;
...
for k = nin:nit
    ...
    LB = [-inf*ones(Nu,1); bandainf(k)*ones(N,1)];
    UB = [inf*ones(Nu,1); bandasup(k)*ones(N,1)];
    X = quadprog(Hqp,fqp1*f,[],[],[],[],LB,UB);
