...
opt1 = optimoptions('quadprog','MaxIterations',6);
...
for k = nin:nit
    ...
    if cont == INTERVALHO_RT_FALHA 
        ...
        if(refs(k)==saidas(k))
            [X2,FVAL,FLAG(k)] = quadprog(Hqp,fqp,[],[],[],[],[],[],[],opt1);
        elseif(refs(k)>=saidas(k))
            [X2,FVAL,FLAG(k)] = quadprog(Hqp,fqp, Rbar,R-f,[],[],[],[],[],opt1);
        else
            [X2,FVAL,FLAG(k)] = quadprog(Hqp,fqp,-Rbar,f-R,[],[],[],[],[],opt1);
        end
...
% ylim([0 1.5])
% h.YTick = [0 0.5 1 1.5];
hl = legend('GPC 2','Referência','Location','NorthEast')
