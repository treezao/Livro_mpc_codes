...
opt1 = optimoptions('quadprog','MaxIterations',3);
...
Fcusto = @(X,H,f) X'*H*X + f'*X; %%% calcula o valor da função custo
...
for k = nin:nit
    ...
    if cont == INTERVALHO_RT_FALHA
        %%% opção 1, zera incremento de controle
        X1 = zeros(Nu,1);
        J(1) = Fcusto(X1,Hqp,fqp);
        
        %%% opção 2, solução sub-ótima
        X2 = quadprog(Hqp,fqp,Rbar,rbar,[],[],LB,UB,[],opt1);
        J(2) = Fcusto(X2,Hqp,fqp);
        
        %%% opção 3, solução irrestrita saturada
        X3 = Kmpc*(R-f);
        ...
        %%% satura sinal de controle
        utemp = entradas(k-1)+X3(1);
        if(utemp>umax)
            X3(1) = umax-entradas(k-1);
        elseif(utemp<umin)
            X3(1) = umin-entradas(k-1);
        end
        
        J(3) = Fcusto(X3,Hqp,fqp);
        
        %%% encontra a solução que tem a menor função custo
        [m,mi] = min(J);
...
% ylim([0 1.5])
% h.YTick = [0 0.5 1 1.5];
hl = legend('GPC 4','Referência','Location','NorthEast')
